...
fn create_kitty() {
	new_test_ext().execute_with(|| {
		...
		// create kitty
		assert_ok!(KittiesModule::create_kitty(signer.clone(), KITTY_NAME));
		...
		let kitty = Kitty { name: KITTY_NAME, dna: KittiesModule::random_kitty_dna(&ACCOUNT_ID_1) };
		...
		assert_noop!(
			KittiesModule::create_kitty(signer, KITTY_NAME),
...
fn bred_kitty() {
	new_test_ext().execute_with(|| {
		...
		// parent 相同
		assert_noop!(
			KittiesModule::bred_kitty(signer.clone(), parent_id_0, parent_id_0, KITTY_NAME),
			Error::<Test>::SameParentKittyId
		);
		// parent 不存在
		assert_noop!(
			KittiesModule::bred_kitty(signer.clone(), parent_id_0, parent_id_1, KITTY_NAME),
		...
		let parent_1 =
			Kitty { name: KITTY_NAME, dna: KittiesModule::random_kitty_dna(&ACCOUNT_ID_1) };
		let parent_2 =
			Kitty { name: KITTY_NAME, dna: KittiesModule::random_kitty_dna(&ACCOUNT_ID_1) };

		// bred kitty
		assert_ok!(KittiesModule::bred_kitty(signer, parent_id_0, parent_id_1, KITTY_NAME));
...
fn transfer_kitty() {
	new_test_ext().execute_with(|| {
		...
		// transfer 不存在的 kitty
		assert_noop!(
			KittiesModule::transfer_kitty(signer.clone(), ACCOUNT_ID_2, KITTY_ID_0),
			Error::<Test>::KittyNotExist
		);
		...
		// 非ower进行transfer
		assert_noop!(
			KittiesModule::transfer_kitty(signer_2.clone(), ACCOUNT_ID_1, KITTY_ID_0),
			Error::<Test>::NotKittyOwner
		);

		// transfer 给 ower
		assert_noop!(
			KittiesModule::transfer_kitty(signer.clone(), ACCOUNT_ID_1, KITTY_ID_0),
			Error::<Test>::TransferKittyToOwner
		);

		// transfer 1 -> 2
		assert_ok!(KittiesModule::transfer_kitty(signer, ACCOUNT_ID_2, KITTY_ID_0));
		...
		// transfer 2 -> 1
		assert_ok!(KittiesModule::transfer_kitty(signer_2, ACCOUNT_ID_1, KITTY_ID_0));
		assert_eq!(KittiesModule::kitty_owner(KITTY_ID_0), Some(ACCOUNT_ID_1));
		System::assert_last_event(
			Event::KittyTransferred {
				sender: ACCOUNT_ID_2,
...
fn sale_kitty() {
	new_test_ext().execute_with(|| {
		...
		// sale 不存在的 kitty
		assert_noop!(
			KittiesModule::sale_kitty(signer.clone(), KITTY_ID_0),
			Error::<Test>::KittyNotExist
		);

		// sale 不属于自己的 kitty
		assert_ok!(KittiesModule::create_kitty(signer.clone(), KITTY_NAME));
		assert_noop!(KittiesModule::sale_kitty(signer_2, KITTY_ID_0), Error::<Test>::NotKittyOwner);

		// sale 上架成功
		assert_ok!(KittiesModule::sale_kitty(signer.clone(), KITTY_ID_0));
		System::assert_last_event(
		...
		// 重复上架
		assert_noop!(
			KittiesModule::sale_kitty(signer, KITTY_ID_0),
...
fn buy_kitty() {
	new_test_ext().execute_with(|| {
		...
		// buy 不存在的 kitty
		assert_noop!(
			KittiesModule::buy_kitty(signer.clone(), KITTY_ID_0),
			Error::<Test>::KittyNotExist
		);
		...
		// buy 未上架的 kitty
		assert_noop!(
			KittiesModule::buy_kitty(signer_2.clone(), KITTY_ID_0),
			Error::<Test>::KittyNotOnSale
		);

		// buy 成功
		assert_ok!(KittiesModule::sale_kitty(signer.clone(), KITTY_ID_0));
		assert_ok!(KittiesModule::buy_kitty(signer_2.clone(), KITTY_ID_0));
		System::assert_last_event(
