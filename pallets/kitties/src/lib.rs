...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn kitty_buy(
			...
			let price = T::KittyPrice::get();
			//通常情况下，当某个操作需要使用账户中的一定金额时，必须先使用 reserve 方法将这些金额锁定，
			//然后在该操作完成后再使用 unreserve 方法解锁。
			//这样可以确保账户中的余额不会被多次使用或重复使用，从而避免出现不一致的情况。
			//T::Currency::reserve(&who, price)?;
