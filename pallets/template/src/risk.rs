...
/// 信贷返回的用户名称，银行uri, 可贷款金额
#[derive(Deserialize, Encode, Decode, Clone, PartialEq, Eq, TypeInfo)]
pub struct RiskLoanAmount {
	#[serde(deserialize_with = "de_string_to_bounded_bytes")]
	pub name: BoundedVec<u8, ConstU32<32>>,
	#[serde(deserialize_with = "de_string_to_bounded_bytes", rename(deserialize = "finaceCom"))]
	pub finance_com: BoundedVec<u8, ConstU32<32>>,
	#[serde(deserialize_with = "de_string_to_u32", rename(deserialize = "loanamount"))]
	pub loan_amount: u32,
}

impl fmt::Debug for RiskLoanAmount {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		f.debug_struct("RiskLoanAmount")
			.field("name", &sp_std::str::from_utf8(&self.name).map_err(|_| fmt::Error)?)
			.field("finance_com", &sp_std::str::from_utf8(&self.finance_com).map_err(|_| fmt::Error)?)
			.field("loan_amount", &self.loan_amount)
			.finish()
	}
}

/// id 为13000000000 贷款响应
#[derive(Deserialize, Debug, Encode, Decode)]
pub struct RiskLoanAmountResponse {
	pub status: i32,
	#[serde(deserialize_with = "de_vec_to_bounded_vec")]
	pub data: BoundedVec<RiskLoanAmount, ConstU32<10>>,
}
...
{
	let s: &str = Deserialize::deserialize(de)?;
	let amount = s.parse::<f32>().map_err(|_| serde::de::Error::custom("Invalid f32"))?;
	let amount = (amount * 100f32) as u32;
	Ok(amount)
}
...
pub fn de_vec_to_bounded_vec<'de, D>(
	de: D,
) -> Result<BoundedVec<RiskLoanAmount, ConstU32<10>>, D::Error>
where
	D: Deserializer<'de>,
{
	let s: Vec<RiskLoanAmount> = Deserialize::deserialize(de)?;
	let a = s
		.into_iter()
		.take(10)
		.collect::<Vec<RiskLoanAmount>>()
		.try_into()
		.map_err(|_| serde::de::Error::custom("BoundedVec error"))?;
