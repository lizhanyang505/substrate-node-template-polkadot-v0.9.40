...
use sp_std::vec;
	...
	impl <T: Config> ValidateUnsigned for Pallet<T> {
		...
		fn validate_unsigned(_source: TransactionSource, call: &Self::Call) -> TransactionValidity {
			...
			let valid_tx = |provide| ValidTransaction::with_tag_prefix("my_pallet")
							.priority(UNSIGNED_TXS_PRIORITY) //define `UNSIGNED_TXS_PRIORITY` before this line
	...
	impl <T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		...
		fn offchain_worker(block_number: T::BlockNumber) {
			let value: u64 = 42;
			// this is your call to on-chain extrinsic together with necessary parameters.
			...
			_ = SubmitTransaction::<T, Call<T>>::submit_unsigned_transaction(call.into())
										.map_err(|_| {
											log::error!("OCW===> Faild in offchain_unsigned_tx");
										});
